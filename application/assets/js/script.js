...
$(document).ready(function() {
    ...
    function showStationList() {
        ...
        $("form input#station-list").val(last_search)
    ...
    var ac_selected_station = $('#station-list').autocomplete({
        ...
        onSelect: function(suggestion) {
            ...
            localStorage.setItem('last_search', suggestion.value)
        ...
        xhttp.onload = function() {
            ...
            if (xhttp.readyState === xhttp.DONE && xhttp.status === 200) {
                ...
                $(xml).find('StopEventResult').each(function(index) {
                    ...
                    let realTimeDep = $(this).find('ServiceDeparture').children(":first").next().text();
                    let realTimeArr = $(this).find('ServiceArrival').children(":first").next().text();
                    let delay = "-";

                    //delay
                    if (moment(arrivalTime) < moment(realTimeArr)) {

                        let delayInMinute = moment.utc(arrivalTime).local() - moment.utc(realTimeArr).local();
                        delayInMinute = moment(delayInMinute).format("HH:mm")
                        delay = "<h1 class='delay'>Delay: " + delayInMinute + "</h1>";

                    }

                    if (moment(departureTime) < moment(realTimeDep)) {

                        let delayInMinute = moment(realTimeDep).local();
                        delayInMinute = moment(delayInMinute).format("DD.MM.YYYY, HH:mm")
                        delay = "<h1 class='delay'>Delay: " + delayInMinute + "</h1>";
                    ...
                    if (param_arrdep == "arrival") {
                        ...
                        article = "<article  tabindex=" + index + "><h1>" + station + "</h1>" +
                            "<h1>" + moment.utc(arrivalTime).local().format("DD.MM.YYYY, HH:mm") + "</h1>" +

                            delay +

                            "<h1>Plattform: " + bay + "</h1></article>"
                    }

                    if (param_arrdep == "departure") {
                        var station = $(this).find('DestinationText').children(":first").text();
                        article = "<article  tabindex=" + index + "><h1>" + station + "</h1>" +
                            "<h1>" + moment.utc(departureTime).local().format("DD.MM.YYYY, HH:mm") + "</h1>" +
                            delay +
                            "<h1>Plattform: " + bay + "</h1></article>"
                    }
